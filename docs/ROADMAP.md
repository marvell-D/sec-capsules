...
完整的当前行为见 [v0.2.0a2 Agent 基准开发者手册](zh-CN/V0.2.0a2_Agent基准开发者手册.md)。版本号表示开发计划，不代表安全认证或生产级承诺。
...
**v0.2.0a2 已完成基础切片。** FFUF 只使用打包小字典，字典路径、线程数、超时和最大执行时间由 profile 锁定；Agent 只能设置状态码集合和每秒请求数。parser 返回去重 endpoint、evidence 与可见的 parse diagnostics。
...
### 2.1 Agent Integration Benchmark

**v0.2.0a2 已完成第一版。** 顶层 `benchmarks/` 不进入 Runtime 发布包，只作为一个可替换的测试消费者。它用同一 Reference Agent 对比 Raw argv/stdout 与 Capsule integration，记录 provider token usage、原始/可见工具字节、峰值上下文、失败、安全拒绝和 milestone 得分。

- Replay 层使用同一 artifact，隔离工具运行波动。
- Live 层启动锁定提交的本机 crAPI，执行真实 Nmap/FFUF。
- 确定性压力测将 FFUF fixture 放大 100 倍，验证 Observation 大小不随重复 raw records 线性增长。
- 安全拒绝与任务失败分开统计，避免误读 Agent 能力。
