...
总体设计、逐函数代码导读与启动示例见[中文开发者手册](zh-CN/V0.1.1_开发者手册.md)。
...
| `usage` | brief 加上适用/避免场景、profile 摘要、输出声明。 | 选择工具前。 |
| `full` | 原始工具卡完整内容。 | 已决定使用某个工具，需要核对参数和限制。 |

执行完成后，默认返回的重点是 `ObservationPacket`：状态、计数、有限的服务/端点/finding、推荐下一步和 artifact reference。大体积 stdout/stderr 仍在磁盘中；读取它必须给出 `artifact://run_id/artifacts/name#Lx` 并受行数和字符数限制。
...
2. **Capsule profile 门槛**：profile 必须存在；其 `active` 与 `action` 将进入 Scope 判定。
3. **Scope 门槛**：`ScopePolicy` 必须允许目标、解析后的 DNS 地址、活动类型和速率。
4. **审批门槛**：Scope 的 `require_approval_for` 或 profile 的 `requires_approval` 要求时，approval 文件必须绑定当前 action 与 target。
5. **工具预检门槛**：`inspect_tool()` 必须能定位 binary 并运行 version command。
...
| `run_capsule` | `capsule_id`、`target`、`scope_file`、`profile`、`execute`、可选 `fixture`、`approval_file`、`timeout`、`token_budget`。 |
| `run_recipe` | `recipe_id_or_path`、`target`、`scope_file`、`execute` 与相同的执行控制参数。 |
